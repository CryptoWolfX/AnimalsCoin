...
#if defined(NDEBUG)
# error "Animalscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0088570eeaaf3503e65163d1aae6313b2ddcc5e363485b9cf7f0ae8c7e897275");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Animalscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Animalscoin Signed Message:\n";
...
{
    // Animalscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Animalscoin: 1Mil blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan =1 * 24 * 60 * 60; // Animalscoin: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // Animalscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Animalscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Animalscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x26fa453a699cc80737d663058ad84944b2b34f283d7e292371f9b8962d884c2b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Prague Times 19/jan/2018, animals are walking free again  ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048994195b8490ad49b4f6667c3c816a80ba6b1ac0fb3f7e9246255c5a87df06f3c754694d0368b35622c60176b8d16793a677b62a8f1455c86dd709857a223dbe") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516494259;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085823489;

        if (fTestNet)
        {
            block.nTime    = 1516494237;
            block.nNonce   = 386885476;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x38a79d344dfbb9fa2a0272571966f16a3eef551d862022c877f8a73cb8c8b5e3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xcc, 0xbf, 0xd9 }; // Animalscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AnimalscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AnimalscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AnimalscoinMiner(CWallet *pwallet)
{
    printf("AnimalscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("animalscoin-miner");
    ...
    try { loop {
        ...
        printf("Running AnimalscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AnimalscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AnimalscoinMiner, pwallet));
}
